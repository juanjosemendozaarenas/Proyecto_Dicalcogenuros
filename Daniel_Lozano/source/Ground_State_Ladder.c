...
int main(int argc, char **argv)
{
    ...
    tntNodeArray nnl, nnr, nnnl, nnnr, os;          /* operators for building the hamiltonian */
    tntComplexArray nnparam, nnnparam, osparam;   /* parameters for building the hamiltonian */
    ...
    unsigned ignoreQN=0;               /* Set to 1 to use symmetries, 0 otherwise */
    ...
    tntNodeArraysNamedLoad(initfile,nnl,"nnlg",nnr,"nnrg",nnnl,"nnnlg",nnnr,"nnnrg",os,"osg");
    tntComplexArraysNamedLoad(initfile,nnparam,"nnparamg", nnnparam,"nnnparamg",osparam,"osparamg");
    ...
    tntNodeArrayFree(&nnnl);
    tntNodeArrayFree(&nnnr);
    tntNodeArrayFree(&os);
    tntComplexArrayFree(&nnparam);
    tntComplexArrayFree(&nnnparam);
    ...
    for (i = 0; i < i_max; i++) {
        ...
        printf("index number %d in sweeps",i);
